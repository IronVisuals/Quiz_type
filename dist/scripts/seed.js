...
async function seed() {
    try {
        ...
        await (0, db_1.init)();
        // Limpa as tabelas
        await db_1.pool.query('DELETE FROM answers');
        await db_1.pool.query('DELETE FROM questions');
        await db_1.pool.query('DELETE FROM users');
        // Insere as questões e respostas
        for (const q of questions) {
            const qRes = await db_1.pool.query('INSERT INTO questions(text, points) VALUES($1, $2) RETURNING id', [q.text, q.points]);
            const questionId = qRes.rows[0].id;
            for (const a of q.answers) {
                await db_1.pool.query('INSERT INTO answers(question_id, text, is_correct) VALUES($1, $2, $3)', [questionId, a.text, a.is_correct]);
            }
        }
